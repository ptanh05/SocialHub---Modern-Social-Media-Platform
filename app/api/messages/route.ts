...
export async function GET(request: NextRequest) {
  try {
    ...
    const conversations = getConversations(payload.userId);
    const unreadCount = getUnreadMessagesCount(payload.userId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const message = createMessage(payload.userId, receiverId, content);
